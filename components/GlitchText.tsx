...
const GradientText: React.FC<GradientTextProps> = ({ text, as: Component = 'span', className = '' }) => {
  return (
    <Component className={`relative inline-block isolate ${className}`}>
      {/* Inner wrapper ensures absolute layers respect parent padding */}
      <span className="relative block">
        {/* Dynamic Gradient Layer */}
        <motion.span
          className="absolute inset-0 left-auto top-auto z-10 block bg-gradient-to-r from-[#ff6700] via-[#ebebeb] via-[#3a6ea5] via-[#ff6700] bg-[length:200%_auto] bg-clip-text text-transparent"
